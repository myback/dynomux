...
var (
	ErrNilHandler     = errors.New("http: nil handler")
	ErrInvalidPattern = errors.New("http: invalid pattern")
)
...
func (mux *ServeMux) Handler(r *http.Request) (h http.Handler, pattern string) {
	...
	uri := cleanPath(r.URL.Path)

	// If the given path is /tree and its handler is not registered,
	// redirect for /tree/.
	if u, ok := mux.redirectToPathSlash(host, uri, r.URL); ok {
		return http.RedirectHandler(u.String(), http.StatusMovedPermanently), u.Path
	}

	if uri != r.URL.Path {
		_, pattern = mux.handler(host, uri)
		u := &url.URL{Path: uri, RawQuery: r.URL.RawQuery}
...
// RemoveHandler remove the handler function for the given pattern.
func (mux *ServeMux) RemoveHandler(pattern string) error {
	if pattern == "" {
		return ErrInvalidPattern
	}
	...
	if _, ok := mux.m[pattern]; !ok {
		return errors.New("http: unknown pattern " + pattern)
	}

	for i, entry := range mux.es {
		if entry.pattern == pattern {
			mux.es = append(mux.es[:i], mux.es[i+1:]...)
			break
		}
	}
